...
def openData(dataPath):
    ...
    xAxis = cols[0]
    yAxis = cols[1]
    chartType = 'bar_chart' #getChartType(xAxis)
...
years = [str(i) for i in range(1850, 2022)]
...
relationsPath = '../dataset/relations.txt'
with open(relationsPath, 'r', encoding='utf-8') as relationsFile:
    relations = relationsFile.readlines()
mArray = []
templateSummariesPath = '../dataset/templateSummaries.txt'
with open(templateSummariesPath, 'r', encoding='utf-8') as templateSummariesFile:
    templateSummaries = templateSummariesFile.readlines()
templateSummariesArr = []

for m in range(len(dataFiles)):
    m_copy = m
    dataPath = '../dataset/data/' + str(dataFiles[m]) + '.csv'
    captionPath = '../dataset/captions/' + str(captionFiles[m]) + '.txt'
    titlePath = '../dataset/titles/' + str(titleFiles[m]) + '.txt'
    ...
    relation = relations[m_copy]  
    relation_len = len(relation.split(' '))  
    dataLine = dataLine + relation
    mArray.append(m_copy)
    templateSummariesArr.append(templateSummaries[m_copy])
    ...
    add_len = 0
    if(relation_len % 2 == 0):
        add_len = relation_len / 2
    else:
        add_len = math.floor(relation_len/2)
    add_len = int(add_len)
    # print(add_len)
    xDataLabels = [0 for item in range(0, len(xValueArr) + add_len )]
    yDataLabels = [0 for item in range(0, len(yValueArr) + add_len + 1 )]
    labelMap = []
    ...
    # print(len(dataLabelLine.split()), len(xValueArr), len(yValueArr), relation_len)    
    assert len(dataLabelLine.split()) == (len(xValueArr) + len(yValueArr) + relation_len - 1)
...
dataArrShuffled, dataLabelArrShuffled, summaryArrShuffled, summaryLabelArrShuffled, oldSummaryArrShuffled, titleArrShuffled = dataArr, dataLabelArr, summaryArr, summaryLabelArr, oldSummaryArr, titleArr
...
##### FOR IZA SPLIT FIXED IN COLLABORATION 
trainSize = 660
validSize = 213
testSize = 190

trainData = dataArrShuffled[0:trainSize]
validData = dataArrShuffled[trainSize:trainSize + validSize]
testData = dataArrShuffled[trainSize + validSize:]

trainDataLabel = dataLabelArrShuffled[0:trainSize]
validDataLabel = dataLabelArrShuffled[trainSize:trainSize + validSize]
testDataLabel = dataLabelArrShuffled[trainSize + validSize:]

trainSummary = summaryArrShuffled[0:trainSize]
validSummary = summaryArrShuffled[trainSize:trainSize + validSize]
testSummary = summaryArrShuffled[trainSize + validSize:]

trainSummaryLabel = summaryLabelArrShuffled[0:trainSize]
validSummaryLabel = summaryLabelArrShuffled[trainSize:trainSize + validSize]
testSummaryLabel = summaryLabelArrShuffled[trainSize + validSize:]

trainTitle = titleArrShuffled[0:trainSize]
validTitle = titleArrShuffled[trainSize:trainSize + validSize]
testTitle = titleArrShuffled[trainSize + validSize:]

oldTrainSummary = oldSummaryArrShuffled[0:trainSize]
oldValidSummary = oldSummaryArrShuffled[trainSize:trainSize + validSize]
oldTestSummary = oldSummaryArrShuffled[trainSize + validSize:]

trainTemplateSummary = templateSummariesArr[0:trainSize]
validTemplateSummary = templateSummariesArr[trainSize:trainSize + validSize]
testTemplateSummary = templateSummariesArr[trainSize + validSize:]
...
with open('../data/test/testData.txt', mode='wt', encoding='utf8') as myfile2:
    myfile2.writelines("%s" % line for line in testData)
...
with open('../data/valid/validData.txt', mode='wt', encoding='utf8') as myfile4:
    myfile4.writelines("%s" % line for line in validData)
...
with open('../data/train/trainSummary.txt', mode='wt', encoding='utf8') as myfile6:
    myfile6.writelines("%s" % line for line in trainTemplateSummary)
...
with open('../data/test/testSummary.txt', mode='wt', encoding='utf8') as myfile8:
    myfile8.writelines("%s" % line for line in testTemplateSummary)
...
with open('../data/valid/validSummary.txt', mode='wt', encoding='utf8') as myfile10:
    myfile10.writelines("%s" % line for line in validTemplateSummary)
...
# with open('../data/train/trainTemplateSummary.txt', mode='wt', encoding='utf8') as myfile6:
#     myfile6.writelines("%s" % line for line in trainTemplateSummary)
# with open('../data/valid/validTemplateSummary.txt', mode='wt', encoding='utf8') as myfile6:
#     myfile6.writelines("%s" % line for line in validTemplateSummary)
# with open('../data/test/testTemplateSummary.txt', mode='wt', encoding='utf8') as myfile6:
#     myfile6.writelines("%s" % line for line in testTemplateSummary)
